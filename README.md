...
회사 앱 서비스가 호출하는 통합 플랫폼 서버와 사내 공통 External API Gateway의  
설계, 개발, 리팩토링, 운영, 유지보수를 수행하고 있습니다.

충전, 가입자 조회, Mail, 국제 SMS, Push, Daemon, Scheduler, Callback API / Callback Daemon 등  
운영 민감도가 높은 영역을 다루며,  
기능 구현보다 상태 전이, 예외 흐름, 복구 가능성, 최종 정합성이 중요한 문제를 더 중요하게 보고 있습니다.

또한 외부 연동 구조 설계, 레거시 PHP / Java 코드 분석, Spring Boot / Gradle 구조 리빌딩,  
장애 대응과 운영 안정성 개선을 함께 수행하며  
운영형 백엔드 문제를 구조적으로 풀어내는 데 집중해왔습니다.
...
- 국내 주요 산업 현장에서 짧게는 1개월, 길게는 약 1.5년의 장기 출장을 수행했고, 베트남, 중국, 인도, 미국 등 해외 현장 대응 경험도 보유하고 있습니다.
...
- `CommerceOrchestrationService` 중심으로 흐름 제어와 domain 책임을 분리
- settlement failure와 notification failure를 동일하게 취급하지 않고 보상 / 복구 정책을 분리
- outbox retry / dead-letter / admin reprocessing 구조를 통해 운영 복구 가능성을 반영
...
- 공통 응답 구조를 표준화해 호출부 복잡도를 줄이는 방향으로 설계
- 확장 가능한 라우팅 구조를 통해 새로운 Provider 추가 비용을 낮추는 방향으로 정리
...
지원 회사별 강조 포인트를 조정한 제출용 이력서는 별도로 관리하고,  
이 문서는 공통적으로 보여주고 싶은 경력 방향과 대표 경험 중심으로 유지합니다.
